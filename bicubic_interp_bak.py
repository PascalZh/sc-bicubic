...
cl_start = 0
print(f"{cl_start=}")
rand_seq = CL_RNG(filename).gen_seq(bsl * 20,
                                    start=cl_start,
                                    bit_width=bit_width)
...
def upscale_bicubic(data_in, data_out):
    ...
    b = np.zeros((4, 3), dtype=np.uint8)  # 用于暂存x方向插值的结果，所以有四个，每个有3个通道
    ...
    for dy in trange(height_out):
        for dx in range(width_out):
            ...
            for i in range(4):
                fs = np.zeros((4, 3), dtype=np.uint8)
                for j in range(4):
                    sxk = sx + j - 1
                    syk = sy + i - 1
            ...
            for ch in range(3):
                p = cubic_interpolate(dy, b[0, ch], b[1, ch], b[2, ch], b[3,
                                                                          ch])
                data_out[dy, dx, ch] = int(np.clip(p, 0, 255))
